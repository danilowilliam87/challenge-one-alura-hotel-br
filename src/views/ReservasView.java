...
public class ReservasView extends JFrame {
	...
	private Date dateEntrada = new Date();
	private Date dateSaida = new Date();
	private Reserva reserva;
	private LocalDate dataDeEntrada = LocalDate.now();
	private LocalDate dataDeSaida = LocalDate.now();
	...
	public ReservasView() {
		...
		txtDataS.addPropertyChangeListener(new PropertyChangeListener() {
			public void propertyChange(PropertyChangeEvent evt) {
				...
				// txtValor.setText("");
				dateEntrada = txtDataE.getDate();
				dateSaida = txtDataS.getDate();
				dataDeEntrada = dateEntrada.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

				dataDeSaida = dateSaida.toInstant().atZone(ZoneId.systemDefault()).toLocalDate();

				total = reservaService.calcularTotalReserva(dataDeEntrada, dataDeSaida);
				txtValor.setText(total.toString());
//				reserva = new Reserva(dataDeEntrada, dataDeSaida, total);
//				System.out.println("data de entrada : " + dataDeEntrada);
//				System.out.println("data de entrada : " + dataDeSaida);
		...
		btnProximo.addMouseListener(new MouseAdapter() {
			...
			public void mouseClicked(MouseEvent e) {
				if (ReservasView.txtDataE.getDate() != null && ReservasView.txtDataS.getDate() != null
						&& ReservasView.txtValor.getText() != null) {
				    
					RegistroHospede registro = new RegistroHospede();
					String formaDePagamento = ReservasView.txtFormaPagamento.getSelectedItem().toString();
					System.out.println("forma de pagamento : " + formaDePagamento);
					System.out.println("data de entrada : " + dataDeEntrada);
					System.out.println("data de saida : " + dataDeSaida);
					reserva = new Reserva(dataDeEntrada, dataDeSaida, total);
					System.out.println(reserva);

					if (formaDePagamento.contains("Crédito")) {
						reserva.setFormaPagamento(FormaPagamento.CREDITO);
					} else if (formaDePagamento.contains("Débito")) {
						reserva.setFormaPagamento(FormaPagamento.DEBITO);
					} else if (formaDePagamento.contains("Dinheiro")) {
						reserva.setFormaPagamento(FormaPagamento.DINHEIRO);
					}
